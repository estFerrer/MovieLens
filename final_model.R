...
tuning_algo <- function(lambda) 
{
  ...
  # Overall average rating (alpha) is subtracted in order to get the predictor's effect.
  ...
  # Same rule applies, but we also subtract previous effects.
...
#    The function sapply() allow for us to execute tuning_algo() function to each value of lambda.
...
# ------------------------- #
# F) Evaluating on test_set #
# ------------------------- #

# 1. Just as we did before, we calculate every predictor's effect but utilizing L value:
...
# 2. Then we add recalculated effects to our testing set: 
beta_test <- test_set %>%
...
# 3. In case there are events or rating years that are not considered by the train_set,
...
# 4. Predicted ratings are calculated:
beta_test <- beta_test %>% 
...
rm(beta_test,b_movie,b_user,b_yoy,b_year,b_event,b_genre)
gc()

# --------------------------------------- #
# G) Training model on entire edx dataset #
# --------------------------------------- #
...
# 2. Training model with L value (representing the optimal lambda) on the entire edx dataset:
